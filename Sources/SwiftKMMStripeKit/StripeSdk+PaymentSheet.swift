...
//  
//
//  Created by abdul basith on 31/10/24.
//
...
extension StripeSdk {
    public func buildPaymentSheetConfiguration(
            params: NSDictionary
    ...
    public func preparePaymentSheetInstance(
           params: NSDictionary,
           ...
           func handlePaymentSheetFlowControllerResult(result: Result<PaymentSheet.FlowController, Error>, stripeSdk: StripeSdk?) {
               ...
               case .failure(let error):
                   resolve(Errors.createError(ErrorType.Failed, error as NSError) as! [String : Any])
               case .success(let paymentSheetFlowController):
                   ...
                   resolve(Mappers.createResult("paymentOption", result) as! [String : Any])
               }
           }
           ...
           if let paymentIntentClientSecret = params["paymentIntentClientSecret"] as? String {
               if (!Errors.isPIClientSecretValid(clientSecret: paymentIntentClientSecret)) {
                   resolve(Errors.createError(ErrorType.Failed, "`secret` format does not match expected client secret formatting.") as! [String : Any])
                   return
               }
               ...
               guard let intentConfiguration = params["intentConfiguration"] as? NSDictionary else {
                   resolve(Errors.createError(ErrorType.Failed, "One of `paymentIntentClientSecret`, `setupIntentClientSecret`, or `intentConfiguration` is required") as! [String : Any])
                   return
               }
               guard let modeParams =  intentConfiguration["mode"] as? NSDictionary else {
                   resolve(Errors.createError(ErrorType.Failed, "One of `paymentIntentClientSecret`, `setupIntentClientSecret`, or `intentConfiguration.mode` is required") as! [String : Any])
                   return
               }
               if (intentConfiguration.object(forKey: "confirmHandler") == nil) {
                   resolve(Errors.createError(ErrorType.Failed, "You must provide `intentConfiguration.confirmHandler` if you are not passing an intent client secret") as! [String : Any])
                   return
               }
            ...
            return PaymentSheet.IntentConfiguration.init(
                ...
                confirmHandler: { paymentMethod, shouldSavePaymentMethod, intentCreationCallback in
                    if (self.hasEventListeners) {
                        ...
                        print("Need to implement an event listener")
...
//                        ])
                        ...
                        print("Tried to call confirmHandler, but no callback was found. Please file an issue: https://github.com/stripe/stripe-react-native/issues")
                    }
                })
        }
...
//                        self.sendEvent(withName: "onOrderTrackingCallback", body: [:])
                        ...
                        print("Order tracking is enabled, but no callback was found. Please file an issue: https://github.com/stripe/stripe-react-native/issues")
                    }
                } else {
                    completion(result)
                }
